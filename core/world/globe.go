...
func (g *Grid) Norm(p entity.Pos) entity.Pos {
	if g.Wrap {
		p.X = g.wrapX(p.X)
	}
	return p
...
func (g *Grid) Index(p entity.Pos) int {
	if g.Wrap {
...
func (g *Grid) PosOf(i int) entity.Pos {
	return entity.Pos{X: i % g.W, Y: i / g.W}
}

// Dist is the Chebyshev distance between two positions, going round the
// map where that is shorter.
func (g *Grid) Dist(a, b entity.Pos) int {
...
func (g *Grid) Delta(from, to entity.Pos) entity.Pos {
	d := entity.Pos{X: to.X - from.X, Y: to.Y - from.Y}
	if g.Wrap {
		d.X %= g.W
		if d.X > g.W/2 {
			d.X -= g.W
...
func (g *Grid) Toward(from, to entity.Pos) entity.Pos {
	d := g.Delta(from, to)
	return g.Norm(entity.Pos{X: from.X + sign(d.X), Y: from.Y + sign(d.Y)})
}
