...
func TestRiversRunOffTheMap(t *testing.T) {
	...
	for i := range g.Tiles {
		if f := g.Tiles[i].Flow; f > most {
			start, most = entity.Pos{X: i % g.W, Y: i / g.W}, f
		}
	}
	p := start
	for step := 0; step < len(g.Tiles); step++ {
		...
		p = entity.Pos{X: p.X + a.X, Y: p.Y + a.Y}
	}
...
func TestSlopeAndAspectReadTheRamp(t *testing.T) {
	...
	for y := 0; y < g.H; y++ {
		for x := 0; x < g.W; x++ {
			g.At(entity.Pos{X: x, Y: y}).Height = float64(10 - x) // falls eastward
		}
	}
	...
	for y := 0; y < g.H; y++ {
		for x := 0; x < g.W; x++ {
			g.At(entity.Pos{X: x, Y: y}).Height = float64(y) * 8 // falls northward
		}
	}
	shaded := g.Sunlight(entity.Pos{X: 5, Y: 2})
	for y := 0; y < g.H; y++ {
		for x := 0; x < g.W; x++ {
			g.At(entity.Pos{X: x, Y: y}).Height = float64(g.H-y) * 8 // falls southward
		}
	}
	if sunny := g.Sunlight(entity.Pos{X: 5, Y: 2}); sunny <= shaded {
...
func TestClimbingCostsMoreThanContouring(t *testing.T) {
	...
	for y := 0; y < g.H; y++ {
		for x := 0; x < g.W; x++ {
			...
			g.At(entity.Pos{X: x, Y: y}).Height = math.Max(0, 40-4*math.Abs(float64(x-10)))
		}
	}
	level := g.StepCost(entity.Pos{X: 1, Y: 5}, entity.Pos{X: 1, Y: 6})
	up := g.StepCost(entity.Pos{X: 5, Y: 5}, entity.Pos{X: 6, Y: 5})
	down := g.StepCost(entity.Pos{X: 6, Y: 5}, entity.Pos{X: 5, Y: 5})
	...
	over := g.TravelCost(entity.Pos{X: 4, Y: 10}, entity.Pos{X: 16, Y: 10})
	along := g.TravelCost(entity.Pos{X: 1, Y: 4}, entity.Pos{X: 1, Y: 16})
	if over <= along {
...
func TestTheSameSeedRaisesTheSameGround(t *testing.T) {
	a, b := NewSized(11, 40, 30), NewSized(11, 40, 30)
