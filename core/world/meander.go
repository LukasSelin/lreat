...
// turning away from.
func turn(d entity.Pos, left bool) entity.Pos {
	if left {
		return entity.Pos{X: d.Y, Y: -d.X}
	}
	return entity.Pos{X: -d.Y, Y: d.X}
}
...
func (g *Grid) meander(by float64) {
	...
	from := make([]entity.Pos, len(g.Tiles))
	...
	for i := range g.Tiles {
		...
		q := entity.Pos{X: p.X + d.X, Y: p.Y + d.Y}
		if g.Wrap {
			q = g.Norm(q)
		}
		if !g.In(q) {
...
func (g *Grid) shift(p, off entity.Pos, by float64, change []float64) bool {
...
func (g *Grid) rockAt(p, off entity.Pos) float64 {
	q := entity.Pos{X: p.X + off.X, Y: p.Y + off.Y}
	if g.Wrap {
		q = g.Norm(q)
	}
