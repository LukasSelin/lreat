...
func TestWaterLeavesDownTheSteepestFall(t *testing.T) {
	...
	for y := 0; y < g.H; y++ {
		for x := 0; x < g.W; x++ {
			g.At(entity.Pos{X: x, Y: y}).Height = float64(40 - x)
		}
	}
	if got := g.Aspect(entity.Pos{X: 10, Y: 10}); got != (entity.Pos{X: 1}) {
	...
	for y := 0; y < g.H; y++ {
		for x := 0; x < g.W; x++ {
			g.At(entity.Pos{X: x, Y: y}).Height = float64(80 - x - y)
		}
	}
	if got := g.Aspect(entity.Pos{X: 10, Y: 10}); got != (entity.Pos{X: 1, Y: 1}) {
...
func TestARiverRunsAllTheWayDown(t *testing.T) {
	for _, cfg := range []Config{DefaultConfig(), smallGlobe()} {
		g := NewWith(3, cfg).Grid
		broken, rivers := 0, 0
		for i := range g.Tiles {
...
func TestRiversHeadInTheHighGround(t *testing.T) {
	for _, seed := range []uint64{1, 2, 3} {
		g := NewWith(seed, DefaultConfig()).Grid
