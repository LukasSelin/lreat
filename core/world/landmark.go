...
func (g *Grid) Landmarks() []entity.Pos { return g.landmarks.at }
...
// dry tiles.
func (g *Grid) chooseLandmarks(at []entity.Pos) []entity.Pos {
	n := len(g.Tiles)
	least := make([]int, n)
	middle := entity.Pos{X: g.W / 2, Y: g.H / 2}
	for i := range least {
...
func (r *Router) guessFor(from, stop entity.Pos, laden bool, x0, y0, span int) (q guess, none bool) {
	g := r.g
...
func (r *Router) AtLeast(from, to entity.Pos) float64 {
	g := r.g
