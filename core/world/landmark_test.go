...
func TestLandmarkBoundsNeverOverstateTheWalk(t *testing.T) {
	...
	for len(pairs) < 600 {
		p := pair{
			from: entity.Pos{X: rng.Intn(g.W), Y: rng.Intn(g.H)},
			to:   entity.Pos{X: rng.Intn(g.W), Y: rng.Intn(g.H)},
		}
		if rng.Intn(2) == 0 {
			p.load = 1
		}
	...
	route := func(p pair) (cost float64, step entity.Pos, path []entity.Pos, work int) {
		r.Reset()
	...
	laid, cleared := 0, 0
	for x := 5; x < g.W-5; x++ {
		p := entity.Pos{X: x, Y: g.H / 2}
		if t := g.At(p); !t.Wet() && t.Structure == None {
			g.Build(p, Road)
			laid++
		}
	}
...
func TestLandmarksAreTakenAgainOnceTheGroundIsMuchCheaper(t *testing.T) {
	...
	for x := 0; x < 4; x++ {
		g.Build(entity.Pos{X: x, Y: 3}, Road)
	}
	g.RefreshLandmarks(LandmarkRest)
	if g.landmarks.Builds != 1 {
...
func TestSlackIsReadRoundTheSeam(t *testing.T) {
	...
	g.Wrap = true
	g.RefreshLandmarks(0)
	g.Build(entity.Pos{X: 2, Y: 10}, Road)   // chunk column 0
	g.Build(entity.Pos{X: 250, Y: 10}, Road) // chunk column 3
