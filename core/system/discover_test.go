...
func TestFieldsOutOfTheRiversReachTeachIrrigation(t *testing.T) {
	...
	for i := range w.Grid.Tiles {
		...
		if t.Terrain == world.Grass && entity.Dist(p, w.MarketPos) < 5 && n < 4 {
			...
			w.Grid.Turn(p, world.Field)
			w.Grid.Fertility[i], w.Grid.Rich[i] = 0.9, 0.9
...
func TestFieldsDoNotWearOut(t *testing.T) {
	...
	worn, n := meanOf(w, func(i int, t *world.Tile) bool { return t.Is(ontology.Field) && w.Grid.Rich[i] > 0 },
		func(i int, _ *world.Tile) float64 { return w.Grid.Fertility[i] / w.Grid.Rich[i] })
	if n < 3 {
